...
func TreeSort(model any, key string) func(ctx echo.Context) error {
	return func(ctx echo.Context) error {
		...
		// 查询当前节点数据
		info := map[string]any{}
		registry.Db.Model(model).Find(&info, params.ID)

		// 获取新位置上级ID
		var parentId int
		node := map[string]any{}
		if params.Before != 0 {
			err = registry.Db.Model(model).Find(&node, params.Before).Error
			if err != nil {
				return err
			}
...
func (t *TreeSortT) UpdateNodeIds(ids []int, id int, parentId int) error {
	for i, v := range ids {
		...
		err := registry.Db.Model(t.model).Where(t.key+" = ?", v).Updates(data).Error
		if err != nil {
			return err
		}
	}
...
func (t *TreeSortT) GetNodeIds(parentId int) ([]int, error) {
	ids := []int{}
	modelDB := registry.Db.Model(t.model)
	if parentId == 0 {
